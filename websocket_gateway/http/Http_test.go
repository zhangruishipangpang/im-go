package http

import (
	"fmt"
	"github.com/changan/websocket_gateway/register"
	"strings"
	"testing"
)

func TestInvokeRequest(t *testing.T) {

	header := make(map[string]string, 3)
	header["token"] = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..G32br6PTm_EEW6R02l_Ws8SRqt_o8fl59ATwGqPdd5tSSJeWKMVdebuQWvMHy5HzzUO7cl6rwtYoflh9UgwhQnNY_ivYGM_SZVGLss1625WS87fySlEn17hNnkU461bxdJKSBIzKrDE-zfJ-Ody_RggaTcNPGpZVeE2ectHy9ZUUPQB_aUznaWZxolkN2eZ1wgb2Ruq7APsqqOr70IFs55c8mCsmhlbd8yBenEPLwJP2igyGcPRsmKQqdEnsqk1NjIBXh_UPVwlXbPVOa0Y-Q5GzTwPAYtoMIDue-G3U1kBNVwDNUiPVGRN7ZlG92Ivk5CFe5td_WjHGXLj0WWLzKw"
	param := "token=eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..G32br6PTm_EEW6R02l_Ws8SRqt_o8fl59ATwGqPdd5tSSJeWKMVdebuQWvMHy5HzzUO7cl6rwtYoflh9UgwhQnNY_ivYGM_SZVGLss1625WS87fySlEn17hNnkU461bxdJKSBIzKrDE-zfJ-Ody_RggaTcNPGpZVeE2ectHy9ZUUPQB_aUznaWZxolkN2eZ1wgb2Ruq7APsqqOr70IFs55c8mCsmhlbd8yBenEPLwJP2igyGcPRsmKQqdEnsqk1NjIBXh_UPVwlXbPVOa0Y-Q5GzTwPAYtoMIDue-G3U1kBNVwDNUiPVGRN7ZlG92Ivk5CFe5td_WjHGXLj0WWLzKw"
	InvokeRequest(
		"http://127.0.0.1:8081/auth/check_token?"+param,
		"POST",
		strings.NewReader(param),
		header,
	)

}

type Body struct {
	token string `json:"token"`
}

func TestInvokeRequestFromServiceName(t *testing.T) {

	client := register.GetNacosClient()
	client.RegisterInstance()
	body := Body{token: "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..G32br6PTm_EEW6R02l_Ws8SRqt_o8fl59ATwGqPdd5tSSJeWKMVdebuQWvMHy5HzzUO7cl6rwtYoflh9UgwhQnNY_ivYGM_SZVGLss1625WS87fySlEn17hNnkU461bxdJKSBIzKrDE-zfJ-Ody_RggaTcNPGpZVeE2ectHy9ZUUPQB_aUznaWZxolkN2eZ1wgb2Ruq7APsqqOr70IFs55c8mCsmhlbd8yBenEPLwJP2igyGcPRsmKQqdEnsqk1NjIBXh_UPVwlXbPVOa0Y-Q5GzTwPAYtoMIDue-G3U1kBNVwDNUiPVGRN7ZlG92Ivk5CFe5td_WjHGXLj0WWLzKw"}
	responseBody := InvokeRequestFromServiceName("auth-serve", "/auth/check_token", body, nil)

	fmt.Println("打印结果 : ", responseBody)
}
